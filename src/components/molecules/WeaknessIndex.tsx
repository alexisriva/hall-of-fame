...
import { useTeamAnalysis, type MultiplierBreakdown } from "../../hooks/useTeamAnalysis";
import { capitalize } from "../../utils/helpers";

const BREAKDOWN_LABELS: { key: keyof MultiplierBreakdown; label: string; multiplier: string }[] = [
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between gap-4 pb-1.5 border-b border-white/8">
        <span className="text-white/40 text-xs">W<sub>t</sub> score</span>
        <span className="text-white/80 text-xs font-bold tabular-nums">
          {score % 1 === 0 ? score : score.toFixed(2)}
          <span className="text-white/30 font-normal"> / {neutral} neutral</span>
        </span>
      </div>
        ...
        {rows.map(({ key, label, multiplier }) => (
          <div key={key} className="flex items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-2">
              <span className="text-white/25 text-xs tabular-nums">×{multiplier}</span>
      ...
      <div className="flex flex-col gap-3">
        ...
        {typeEntries.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {typeEntries.map(({ type, score, neutral, category, severe, caution, breakdown }) => (
              <div
                key={type}
                className={[
                  ...
                  >
                    {severe || caution ? (
                      <HiExclamationTriangle
                        size={13}
                        className={severe ? "text-[#b22200]" : "text-amber-400"}
                      />
                    ) : (
                      <HiCheckCircle
                        size={13}
